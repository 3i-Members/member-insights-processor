...
class ConfigLoader:
    ...
    def get_context_file_path(self, eni_type: str, eni_subtype: str) -> Optional[str]:
        ...
        try:
            ...
            if not eni_subtype or eni_subtype.strip() == '' or eni_subtype.lower() in ['none', 'nan']:
            ...
            if normalized_subtype in type_mappings:
                ...
                logger.debug(f"Found exact mapping for {eni_type}/{normalized_subtype}: {file_path}")
                return file_path
            ...
            if normalized_subtype == 'null' and 'default' in type_mappings:
                ...
                logger.info(f"Using default mapping for null subtype {eni_type}/{eni_subtype}: {file_path}")
                return file_path
            ...
            logger.warning(f"No mapping found for {eni_type}/{eni_subtype} and no default available")
            return None
        ...
        try:
            ...
            if normalized_subtype in type_mappings:
                ...
                if subtype_path != result['default']:
                    ...
                    logger.debug(f"Found subtype context for {eni_type}/{normalized_subtype}: {result['subtype']}")
        ...
        try:
                ...
                for eni_type, subtypes in eni_mappings.items():
                    ...
                    for subtype, file_path in subtypes.items():
                        if not Path(file_path).exists():
                            report['warnings'].append(f"Context file not found: {file_path} (for {eni_type}/{subtype})")
            ...
            if not system_prompts:
                ...
                for prompt_key, file_path in system_prompts.items():
                    if not Path(file_path).exists():
                        report['warnings'].append(f"System prompt file not found: {file_path} (for key '{prompt_key}')")
            ...
            missing_bq_fields = [field for field in required_bq_fields if not bigquery_config.get(field)]
            
            if missing_bq_fields:
                report['issues'].append(f"Missing required BigQuery configuration fields: {missing_bq_fields}")
            ...
            logger.info(f"Configuration validation complete: {len(report['issues'])} issues, {len(report['warnings'])} warnings")
            return report
