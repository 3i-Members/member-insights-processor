...
class ProcessingFilter:
    ...
    def _load_filter_config(self) -> None:
        ...
        try:
            if not self.filter_config_path.exists():
                raise FileNotFoundError(f"Processing filter file not found: {self.filter_config_path}")
            ...
            logger.error(f"Failed to load processing filter from {self.filter_config_path}: {str(e)}")
            raise
        ...
        try:
            ...
            if eni_type not in self.processing_rules:
                if self.processing_settings.get('log_skipped_records', False):
                    logger.debug(f"Skipping {eni_type}/{eni_subtype}: ENI type not in processing rules")
                return False
            ...
            if isinstance(rule, list):
                ...
                if not should_process and self.processing_settings.get('log_skipped_records', False):
                    logger.debug(f"Skipping {eni_type}/{eni_subtype}: subtype not in allowed list {rule}")
    ...
    def filter_dataframe(self, df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        try:
            ...
            if self.processing_settings.get('show_processing_stats', False):
                logger.info(f"Processing filter results: {filtered_count}/{original_count} records included ({stats['filter_efficiency']:.1f}%)")
                
                if skipped_count > 0:
                    # Show breakdown of skipped records by type
                    skipped_df = df[~mask]
                    skipped_breakdown = skipped_df.groupby(['eni_source_type', 'eni_source_subtype']).size().to_dict()
                    logger.info(f"Skipped records breakdown: {dict(list(skipped_breakdown.items())[:10])}")  # Show first 10

            return filtered_df, stats

        except Exception as e:
            logger.error(f"Error filtering DataFrame: {str(e)}")
            return df, {'original_count': len(df), 'filtered_count': len(df), 'skipped_count': 0, 'error': str(e)}
    ...
    def validate_filter_against_data(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if missing_in_data:
                validation_results['warnings'].append(f"Configured types not found in data: {missing_in_data}")

            # Check for data types not configured
            unconfigured_types = data_types - configured_types
            if unconfigured_types:
                validation_results['warnings'].append(f"Data types not configured for processing: {unconfigured_types}")
            ...
            for eni_type in configured_types & data_types:
                ...
                if isinstance(allowed_subtypes, list):
                    ...
                    actual_subtypes = set(df[df['eni_source_type'] == eni_type]['eni_source_subtype'].fillna('null').astype(str))
                    
                    # Normalize null representations
                    normalized_actual = set()
                    for subtype in actual_subtypes:
                        if not subtype or subtype.strip() == '' or subtype.lower() in ['none', 'nan']:
