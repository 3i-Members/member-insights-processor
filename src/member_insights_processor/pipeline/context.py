...
class ContextManager:
    ...
    def __init__(
        self, config_file_path: str = "config/config.yaml", supabase_client: Optional[Any] = None
    ):
        ...
        self.max_new_data_tokens_per_group: int = int(
            processing_cfg.get("max_new_data_tokens_per_group", 12000)
        )
    ...
    def get_context_file_paths(
        self, eni_source_type: str, eni_source_subtype: Optional[str]
    ...
    def get_current_structured_insight(self, contact_id: str, system_prompt_key: str) -> str:
        ...
        if not self.supabase_client:
            return _as_json()
        try:
            existing = self.supabase_client.get_latest_insight_by_contact_id(
    ...
    # -----------------------------
    def _build_new_data_block(
        self, eni_rows: List[Dict[str, Any]], available_tokens: int, source_type_main: str
    ) -> Tuple[str, int]:
        lines: List[str] = []
        used_tokens = 0
        rows_used = 0
        for row in eni_rows:
            ...
            date_str = (
                date_str_raw
                if date_str_raw and date_str_raw.lower() not in ("nan", "nat", "none", "null")
                else "N/A"
            )
    ...
    def build_context_variables(
        ...
        current_structured_insight = self.get_current_structured_insight(
        ...
        new_data_block, rows_used = self._build_new_data_block(
            eni_rows, available_for_new_data, eni_source_type
        )
                        ...
                        try:
                            ...
                            if file_size == 0:
                                ...
                                report["statistics"]["large_files"].append(
                                    {"file": str(md_file), "size_kb": round(file_size / 1024, 2)}
                                )
