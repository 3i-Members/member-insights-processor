...
class BigQueryConnector:
    ...
    def __init__(
        ...
        """
        self.project_id = (
            project_id or os.getenv("PROJECT_ID") or os.getenv("GOOGLE_CLOUD_PROJECT_ID")
        )
    ...
    def _load_credentials(self) -> Optional[service_account.Credentials]:
        ...
        if missing:
            logger.error(
                f"Missing required Google Cloud credential components: {', '.join(missing)}"
            )
            logger.error("Please set all GCP_* environment variables in your .env file")
            logger.error(
                "See .env.example for guidance on extracting values from service account JSON"
            )
            return None
        ...
        try:
            ...
            credentials = service_account.Credentials.from_service_account_info(
                service_account_info, scopes=["https://www.googleapis.com/auth/bigquery"]
            )
    ...
    def get_contact_ids_from_sql(
        ...
        logger.info(f"Executing contact selection SQL (limit={limit}, offset={offset})")
    ...
    def _build_eni_filter_clause(
        self, eni_source_type: str, eni_source_subtype: Optional[str] = None
    ) -> str:
        """
    ...
    def load_contact_data_filtered(
        self, contact_id: str, eni_source_type: str, eni_source_subtype: Optional[str] = None
    ) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info(
                f"Loading contact data for: {contact_id}, eni_source_type: {eni_source_type}{subtype_desc}"
            )
            ...
            logger.info(
                f"Loaded {len(df)} records for {contact_id}, {eni_source_type}{subtype_desc}"
            )
            ...
            logger.error(
                f"Error loading contact data for {contact_id}, {eni_source_type}: {str(e)}"
            )
            raise
    ...
    def get_prioritized_contact_ids(
        ...
        try:
            ...
            cutoff_expr = (
                f"TIMESTAMP('{cutoff_time_iso}')"
                if cutoff_time_iso
                else "TIMESTAMP(DATETIME_SUB(CURRENT_DATETIME(), INTERVAL 7 DAY))"
            )
    ...
    def get_eni_combinations_for_processing(
        ...
        for eni_source_type, rule in processing_rules.items():
            if rule == "none" or rule is None:
                ...
                logger.debug(
                    f"Adding NULL subtype for {eni_source_type} (no explicit subtypes defined)"
                )
                ...
                combinations.append((eni_source_type, "null"))
                logger.warning(
                    f"Invalid rule for {eni_source_type}: {rule}. Processing NULL subtypes only."
                )

        logger.info(
            f"Generated {len(combinations)} eni_source_type/subtype combinations for processing (NULL subtypes always included)"
        )
        return combinations
    ...
    def get_processed_eni_ids(self, contact_id: str) -> List[str]:
        ...
        try:
            ...
            logger.debug(
                f"Found {len(processed_eni_ids)} processed ENI IDs for contact {contact_id}"
            )
    ...
    def mark_eni_processed(
        ...
        try:
            ...
            duration_value = (
                processing_duration_ms if processing_duration_ms is not None else "NULL"
            )
    ...
    def batch_mark_processed(self, records: List[Dict[str, Any]]) -> Tuple[int, int]:
        """
        ...
        try:
            ...
            for record in records:
                ...
                row = {
                    ...
                    "processing_mode": (
                        "batch"
                        if isinstance(metadata, dict) and metadata.get("batch_id")
                        else "single"
                    ),
    ...
    def get_processing_statistics(self, contact_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            stats = {"by_status": {}, "total_processed": 0, "contact_filter": contact_id}
            ...
            for row in results:
                stats["by_status"][row.processing_status] = {
                    ...
                    "earliest_processed": (
                        row.earliest_processed.isoformat() if row.earliest_processed else None
                    ),
                    "latest_processed": (
                        row.latest_processed.isoformat() if row.latest_processed else None
                    ),
                }
...
def create_bigquery_connector(config: dict = None) -> BigQueryConnector:
    ...
    if not all([project_id, dataset_id, table_name]):
        raise ValueError(
            "Missing required BigQuery configuration: project_id, dataset_id, table_name"
        )
