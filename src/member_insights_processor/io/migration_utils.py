...
from .schema import StructuredInsight, normalize_insight_data, validate_structured_insight_json
...
class MigrationManager:
    ...
    def migrate_all_files(
        self, force_overwrite: bool = False, batch_size: int = 10
    ) -> ProcessingState:
        """
    ...
    def _backup_file(self, file_path: Path) -> None:
        ...
        try:
            ...
            if backup_path.exists():
                ...
                backup_path = (
                    self.backup_directory / f"{name_parts[0]}_{name_parts[1]}{name_parts[2]}"
                )
    ...
    def get_migration_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "success_rate": len(self.migrated_files)
            / max(1, len(self.migrated_files) + len(self.failed_files)),
...
def run_migration_cli():
    ...
    parser.add_argument(
        "--source-dir",
        default="var/output/structured_insights",
    ...
    try:
        ...
        if not client.create_table_if_not_exists():
            logger.error(
                "Supabase table does not exist. Please run the schema creation script first."
            )
            sys.exit(1)

        # Create migration manager
        manager = MigrationManager(
            client, source_directory=args.source_dir, backup_directory=args.backup_dir
        )
        ...
        if args.dry_run:
            ...
            state = manager.migrate_all_files(
                force_overwrite=args.force, batch_size=args.batch_size
            )
