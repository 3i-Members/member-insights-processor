...
class SupabaseAirtableSync:
    ...
    def sync_contact_to_airtable(self, contact_id: str, force_update: bool = False) -> SyncResult:
        ...
        try:
            ...
            insight = self.supabase_client.get_latest_insight_by_contact_id(
            ...
            sync_res = self.airtable_writer.create_note_submission_record(
                contact_id=contact_id, structured_json=airtable_data
            )

            if sync_res and getattr(sync_res, "success", False):
                action = (
                    "created"
                    if getattr(sync_res, "created", False)
                    else ("updated" if getattr(sync_res, "updated", False) else "created")
                )
                return SyncResult(
                    contact_id=contact_id,
                    success=True,
                ...
                return SyncResult(
                    ...
                    error_message=getattr(
                        sync_res, "error", "Failed to create/update Airtable record"
                    ),
                )
            ...
            return SyncResult(
                contact_id=contact_id, success=False, action="failed", error_message=str(e)
            )

    def sync_recent_insights(
        self, hours_back: int = 24, max_records: int = 100, force_update: bool = False
    ) -> List[SyncResult]:
        """
        ...
        try:
            ...
            recent_insights = self.supabase_client.list_insights(
                limit=max_records, order_by="updated_at", ascending=False
            )
            ...
            for insight in filtered_insights:
                result = self.sync_contact_to_airtable(
                    insight.metadata.contact_id, force_update=force_update
                )
    ...
    def sync_all_insights(
        self, batch_size: int = 50, force_update: bool = False
    ) -> List[SyncResult]:
        """
        ...
        try:
            ...
            while offset < total_count:
                ...
                logger.info(
                    f"Processing batch {offset//batch_size + 1}: {len(batch_insights)} insights"
                )
    ...
    def sync_specific_contacts(
        self, contact_ids: List[str], force_update: bool = False
    ) -> List[SyncResult]:
        """
        ...
        insights_dict.update(
            {
                "metadata": {
                    ...
                    "generated_at": (
                        insight.metadata.generated_at.isoformat()
                        if insight.metadata.generated_at
                        else None
                    ),
    ...
    def get_sync_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "recent_summary": (
                self._get_sync_summary(self.sync_results[-100:]) if self.sync_results else {}
            ),
            "overall_summary": (
                self._get_sync_summary(self.sync_results) if self.sync_results else {}
            ),
        }
...
class SupabaseAirtableBridge:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # Initialize clients
        self.supabase_client = SupabaseInsightsClient(**config.get("supabase", {}))

        self.airtable_writer = StructuredInsightsAirtableWriter(config.get("airtable", {}))
