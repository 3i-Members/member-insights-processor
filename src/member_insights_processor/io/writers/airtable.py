...
class StructuredInsightsAirtableWriter:
    ...
    def _initialize_connection(self) -> bool:
        ...
        try:
            ...
            note_table_id = self.config["structured_insight"]["tables"]["note_submission"][
                "table_id"
            ]
    ...
    def _get_airtable_record_id(self, table: Table, field_id: str, value: str) -> Optional[str]:
        ...
        try:
            ...
            if record:
                logger.info(
                    f"Found existing record in table {table.name} where {field_id} = {value}. Record ID: {record['id']}"
                )
                return record["id"]
            ...
            logger.error(
                f"Error looking up Airtable record ID for value {value} in table {table.name}: {e}"
            )
            return None
    ...
    def find_master_record_by_contact_id(self, contact_id: str) -> Optional[str]:
        ...
        contact_field_id = self.config["structured_insight"]["tables"]["master"]["fields"][
            "contact_id"
        ]

        # Find the record
        master_record_id = self._get_airtable_record_id(
            self.master_table, contact_field_id, contact_id
        )
    ...
    def process_structured_json(self, json_data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if "introductions" in json_data and json_data["introductions"]:
            processed_data["introductions"] = self._format_markdown_for_airtable(
                json_data["introductions"]
            )
    ...
    def create_note_submission_record(
        self, contact_id: str, structured_json: Dict[str, Any], member_name: Optional[str] = None
        ...
        if not self.connected:
            return StructuredSyncResult(
                success=False, contact_id=contact_id, error="Not connected to Airtable"
            )
        ...
        try:
            ...
            record_data = {
                ...
                fields["note_submission_type"]: self.config["structured_insight"]["tables"][
                    "note_submission"
                ]["status_column_value"]["elvis"],
            }
            ...
            created_record = self._retry_with_backoff(
                self.note_submission_table.create, record_data
            )

            logger.info(
                f"Created note submission record for {contact_id} ({member_name}): {created_record['id']}"
            )
            ...
            return StructuredSyncResult(success=False, contact_id=contact_id, error=error_msg)

    def sync_structured_insights_batch(
        self, insights_data: List[Dict[str, Any]], show_progress: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        for i, insight_data in enumerate(insights_data, 1):
            ...
            sync_result = self.create_note_submission_record(
                contact_id=contact_id, structured_json=json_data, member_name=member_name
            )
        ...
        results["end_time"] = datetime.now().isoformat()
        logger.info(
            f"Batch sync completed: {results['successful']} successful, {results['failed']} failed"
        )
...
def create_structured_airtable_writer(
    config: Dict[str, Any], api_key: Optional[str] = None
