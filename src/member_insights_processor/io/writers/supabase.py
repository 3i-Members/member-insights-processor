...
class SupabaseInsightsProcessor:
    ...
    def process_insight(
        ...
        try:
            ...
            generator = metadata.get('generator', 'structured_insight') if metadata else 'structured_insight'
            
            # Step 1: Set all previous records for this contact_id + generator to is_latest=false
            try:
                self.client._ensure_connection()
                update_result = self.client._client.table(self.client.TABLE_NAME)\
                    .update({'is_latest': False})\
                    .eq('contact_id', contact_id)\
                    .eq('generator', generator)\
                    .eq('is_latest', True)\
                    .execute()
                logger.debug(f"Set previous records to is_latest=false for {contact_id} + {generator}")
            ...
            try:
                latest_version_result = self.client._client.table(self.client.TABLE_NAME)\
                    .select('version')\
                    .eq('contact_id', contact_id)\
                    .eq('generator', generator)\
                ...
                logger.warning(f"Failed to get latest version for {contact_id}: {e}, defaulting to version 1")
                next_version = 1
            ...
            logger.info(f"Created new versioned insight v{next_version} for contact_id: {contact_id}")
            return result, True
    ...
    def load_existing_insight(self, contact_id: str) -> Optional[StructuredInsight]:
        ...
        # Query from database
        try:
            existing = self.client.get_latest_insight_by_contact_id(contact_id, generator='structured_insight')
            if existing:
