    ...
    def get_current_metrics(self) -> Dict[str, Any]:
        ...
        with self.lock:
            ...
            if self.metrics["ai_calls_made"] > 0:
                ai_success_rate = (
                    self.metrics["ai_calls_successful"] / self.metrics["ai_calls_made"]
                ) * 100
...
class EnhancedLogger:
    ...
    def _setup_loggers(self):
        ...
        self.metrics_handler = logging.handlers.RotatingFileHandler(
            metrics_log_file, maxBytes=5 * 1024 * 1024, backupCount=3  # 5MB
        )
        ...
        ai_handler = logging.handlers.RotatingFileHandler(
            ai_log_file, maxBytes=10 * 1024 * 1024, backupCount=3  # 10MB
        )
        ...
        data_handler = logging.handlers.RotatingFileHandler(
            data_log_file, maxBytes=5 * 1024 * 1024, backupCount=3  # 5MB
        )
    ...
    def log_contact_processing_start(
        self, contact_id: str, total_records: int, filtered_records: int
    ):
        ...
        self.logger.info(
            f"   ✅ Filtered records: {filtered_records} ({(filtered_records/total_records*100):.1f}%)"
        )
        self.data_logger.info(
            f"Contact {contact_id}: {total_records} -> {filtered_records} records after filtering"
        )
    ...
    def log_eni_group_start(
        self, contact_id: str, eni_type: str, eni_subtype: str, record_count: int
    ):
        """Log start of ENI group processing"""
        self.logger.info(
            f"📂 Processing {eni_type}/{eni_subtype} for {contact_id} ({record_count} records)"
        )
        self.data_logger.info(
            f"ENI Group Start: {contact_id} - {eni_type}/{eni_subtype} - {record_count} records"
        )

    def log_eni_group_end(
        self,
        contact_id: str,
        eni_type: str,
        eni_subtype: str,
        ...
        self.metrics.log_eni_group_processing(
            contact_id, eni_type, eni_subtype, record_count, success, duration
        )

        status = "✅" if success else "❌"
        self.logger.info(f"{status} Completed {eni_type}/{eni_subtype} in {duration:.2f}s")
        self.data_logger.info(
            f"ENI Group End: {contact_id} - {eni_type}/{eni_subtype} - Success: {success} - Duration: {duration:.2f}s"
        )

    def log_ai_call_start(self, model: str, eni_type: str, eni_subtype: str, record_count: int):
        """Log start of AI processing call"""
        self.logger.info(
            f"🤖 Calling {model} for {eni_type}/{eni_subtype} ({record_count} records)"
        )
        self.ai_logger.info(
            f"AI Call Start: {model} - {eni_type}/{eni_subtype} - {record_count} records"
        )
    ...
    def log_ai_call_end(
        ...
        status = "✅" if success else "❌"
        self.logger.info(
            f"{status} {model} completed in {duration:.2f}s (response: {response_length} chars)"
        )
        self.ai_logger.info(
            f"AI Call End: {model} - Success: {success} - Duration: {duration:.2f}s - Response length: {response_length}"
        )
    ...
    def log_context_loading(self, eni_type: str, eni_subtype: str, context_files: List[str]):
        ...
        files_str = (
            ...
            else context_files[0] if context_files else "none"
        )
