...
from data_processing.schema import StructuredInsight, normalize_insight_data, validate_structured_insight_json
    ...
    try:
        ...
        if queried_insight:
            ...
            if queried_insight.investing:
                logger.info(f"Investing section length: {len(queried_insight.investing)} characters")
...
def test_table_structure():
    ...
    try:
        ...
        if "relation \"elvis__structured_insights\" does not exist" in str(e):
            logger.error("❌ The table 'elvis__structured_insights' does not exist in your Supabase database")
