...
def print_test_summary(results, test_name: str, duration: float):
    ...
    print(
        f"   ✅ Successes: {getattr(results, 'success_count', results.testsRun - len(results.failures) - len(results.errors))}"
    )
    ...
    try:
        if args.test:
            ...
            if args.components_only:
                ...
                test_files = [
                    f.name
                    for f in test_dir.glob(args.pattern)
                    if f.is_file() and f.name != "test_runner.py"
                ]
