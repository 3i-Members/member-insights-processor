...
class TestProcessingFilters(unittest.TestCase):
    ...
    def setUp(self):
        ...
        filter_path = os.path.join(
            os.path.dirname(__file__), "..", "config", "processing_filters.yaml"
        )
    ...
    def test_processing_filter_combinations_generation(self):
        ...
        null_combinations = [
            (eni_type, subtype) for eni_type, subtype in combinations if subtype == "null"
        ]

        # Every ENI type should have a NULL combination
        for eni_type in processing_rules.keys():
            null_combo_exists = any(
                combo[0] == eni_type and combo[1] == "null" for combo in combinations
            )
            self.assertTrue(
                null_combo_exists, f"ENI type {eni_type} should have a NULL subtype combination"
            )

        logger.info(
            f"✅ Found {len(null_combinations)} NULL subtype combinations (expected for all ENI types)"
        )
    ...
    def test_contact_data_filtering_by_combination(self):
        ...
        for eni_source_type, eni_source_subtype in combinations:
            logger.info(
                f"📋 Testing combination: {eni_source_type} / {eni_source_subtype or 'NULL'}"
            )
            ...
            try:
                ...
                logger.info(
                    f"  📊 {eni_source_type}/{subtype_display}: {record_count} unprocessed records"
                )
                ...
                if record_count > 0:
                    ...
                    self.assertEqual(
                        len(unique_types),
                        1,
                        f"Should only have one eni_source_type: {eni_source_type}",
                    )
                    self.assertEqual(
                        unique_types[0],
                        eni_source_type,
                        f"All records should have eni_source_type: {eni_source_type}",
                    )
                    ...
                    if eni_source_subtype == "null":
                        ...
                        self.assertEqual(
                            null_count,
                            record_count,
                            "All records should have NULL eni_source_subtype",
                        )
                        ...
                        if len(non_null_data) > 0:
                            ...
                            self.assertIn(
                                eni_source_subtype,
                                unique_subtypes,
                                f"Should contain subtype: {eni_source_subtype}",
                            )
        ...
        try:
            ...
            if len(data) > 0:
                ...
                self.assertEqual(
                    unique_contacts[0],
                    contact_id,
                    f"All records should have contact_id: {contact_id}",
                )
