...
class TestNullSubtypeHandling(unittest.TestCase):
        ...
        for source_type, subtype_input in test_cases:
            with self.subTest(source_type=source_type, subtype=subtype_input):
                ...
                self.assertIsNotNone(
                    file_path, f"Expected file path for {source_type}/{subtype_input}, got None"
                )

                # Verify file exists
                self.assertTrue(
                    Path(file_path).exists(), f"Context file does not exist: {file_path}"
                )
    ...
    def test_data_processing_null_normalization(self):
        ...
        self.assertTrue(
            all(test_data["eni_source_subtype"] == "null"),
    ...
    def test_fallback_hierarchy(self):
        ...
        nonexistent_path = self.config_loader.get_context_file_path(
            source_type, "nonexistent_subtype"
        )
        self.assertIsNotNone(nonexistent_path)

        # Should find specific mapping
        specific_path = self.config_loader.get_context_file_path(
            source_type, "investing_preferences"
        )
